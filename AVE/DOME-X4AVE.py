...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--check", action="store_true", help="validate dependencies and posterior artifact")
    return parser.parse_args()


def check_environment() -> None:
    if not ARTIFACT.is_file():
        raise FileNotFoundError(f"Missing AVE posterior artifact: {ARTIFACT}")
    with np.load(ARTIFACT, allow_pickle=False) as archive:
        required = {"labels", "test_labels", "ROST_oof_audio", "ROST_oof_visual"}
        missing = required.difference(archive.files)
        if missing:
            raise KeyError(f"AVE artifact is missing arrays: {sorted(missing)}")
        print(
            f"AVE comparison check passed: development={len(archive['labels'])} "
